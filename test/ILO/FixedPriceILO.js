...
describe("FixedPriceILO contract", function() {
    ...
    describe("No start date, end date, or soft cap", function() {
        ...
        it("Should initialize", async function () {
            expect(await fixedPriceIlocontract.assetToken()).to.be.equal(token0.address);
            expect(await fixedPriceIlocontract.assetTokenAmount()).to.be.equal(assetTokenAmount);
            expect(await fixedPriceIlocontract.tokensPerEth()).to.be.equal(tokensPerEth);
            expect(await fixedPriceIlocontract.startDate()).to.be.equal(0);
            expect(await fixedPriceIlocontract.endDate()).to.be.equal(0);
            expect(await fixedPriceIlocontract.softCap()).to.be.equal(0);
            expect(await fixedPriceIlocontract.assetSwapExchange()).to.be.equal(assetSwapExchangeContract.address);
            expect(await fixedPriceIlocontract.percentageToLock()).to.be.equal(percentageToLock);
            expect(await fixedPriceIlocontract.wrappedEther()).to.be.equal(wrappedEtherContract.address);
            expect(await fixedPriceIlocontract.creator()).to.be.equal(owner.address);

            expect(await token0.balanceOf(fixedPriceIlocontract.address)).to.be.equal(assetTokenAmount);

            await expect(
                fixedPriceIlocontract.initialize(
                token0.address,
                assetTokenAmount,
                tokensPerEth,
                0,
        ...
        it("Should invest", async function() {
            ...
            await fixedPriceIlocontract.invest({ value: depositAmount });
            
            expect(await ethers.provider.getBalance(fixedPriceIlocontract.address)).to.be.equal(depositAmount);
            expect(await fixedPriceIlocontract.etherDeposited(owner.address)).to.be.equal(depositAmount);

            const assetTokensBought = depositAmount.mul(tokensPerEth).div(parseEther("1"));
            expect(await fixedPriceIlocontract.balanceOf(owner.address)).to.be.equal(assetTokensBought);
            expect(await fixedPriceIlocontract.totalAssetTokensBought()).to.be.equal(assetTokensBought);
        });
